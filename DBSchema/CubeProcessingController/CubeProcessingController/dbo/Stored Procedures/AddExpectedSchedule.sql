...
-- Example:
    /*
        DECLARE @TableList TABLELISTTYPE
        INSERT INTO @TableList
        SELECT 'Test', NULL, NULL

        EXEC AddExpectedSchedule
            'ExternalReference'
            , @TableList
            , 'test'
            , 'description'
    ...
    */           
-- =============================================
CREATE PROCEDURE dbo.AddExpectedSchedule
      @strApplicationInstanceExternalReference  VARCHAR(255)
    , @tblTableList             TABLELISTTYPE READONLY
    , @strName                  VARCHAR(100)
...
BEGIN
    ...
    BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM ExpectedSchedule WHERE Name = @strName)
        ...
        IF NOT EXISTS (SELECT 1 FROM ApplicationInstance WHERE ExternalReference = @strApplicationInstanceExternalReference)
        ...
        INSERT INTO ExpectedScheduleDetail
