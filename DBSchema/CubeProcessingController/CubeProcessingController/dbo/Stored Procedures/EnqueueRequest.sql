...
BEGIN
	...
	select @JobID = ID from Job where ExternalID = convert(uniqueidentifier, @JobReference)
	...
	select @TableID = ID from [Table] where ApplicationID = @ApplicationID and DBTableName = @TableName
	...
	select @PartitionValueRequired = 1 from [Table] where ID = @TableID and IsPartitioned = 1
