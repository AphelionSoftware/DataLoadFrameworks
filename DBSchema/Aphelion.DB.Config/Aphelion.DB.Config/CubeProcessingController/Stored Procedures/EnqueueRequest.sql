...
CREATE PROCEDURE [CubeProcessingController].[EnqueueRequest]
...
BEGIN
	...
	select @JobID = ID from [CubeProcessingController].Job where ExternalID = convert(uniqueidentifier, @JobReference)
	...
	select @ApplicationID = ApplicationID from [CubeProcessingController].ApplicationInstance a, [CubeProcessingController].Job j where a.ID = j.ApplicationInstanceID and j.ID = @JobID
	...
	select @TableID = ID from [CubeProcessingController].[Table] where ApplicationID = @ApplicationID and DBTableName = @TableName
	...
	select @PartitionValueRequired = 1 from [CubeProcessingController].[Table] where ID = @TableID and IsPartitioned = 1
