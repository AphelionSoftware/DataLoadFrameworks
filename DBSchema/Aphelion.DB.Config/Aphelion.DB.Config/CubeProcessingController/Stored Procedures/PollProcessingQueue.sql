...
CREATE PROCEDURE [CubeProcessingController].[PollProcessingQueue]
AS
...
BEGIN
	...
	IF NOT EXISTS (SELECT 1
				   FROM [CubeProcessingController].ProcessQueue 
				   WHERE StatusID = -1
				  ) 
	BEGIN
		...
		INTO #tmpQ
		FROM [CubeProcessingController].ProcessQueue			p
		JOIN [CubeProcessingController].Job					j
			ON j.ID = p.JobID
		JOIN [CubeProcessingController].ApplicationInstance	ai
			ON j.ApplicationInstanceID = ai.ID
		JOIN [CubeProcessingController].[Application]			a
